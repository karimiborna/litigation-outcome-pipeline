...
def _select_best_similar_cases(items: list[SimilarCaseItem]
                               , max_best: int = 3) -> list[SimilarCaseItem]:
    return sorted(
        items,
...
async def _build_similarity_advice(
    ...
    try:
        ...
        return parsed.get("comparison_insights",
                          "Comparison analysis was unavailable."), parsed.get(
            "advice",
...
def _build_frontend_analysis_response(
    ...
    if vector.documentary_evidence:
        strengths.append("You appear to have "
                         "documentary evidence that can support your version of events.")
    elif vector.documentary_evidence is False:
        weaknesses.append("The claim may be harder"
    ...
    if vector.contract_present:
        ...
        weaknesses.append("This dispute may turn on"
                          " oral promises unless you can show clear written terms.")
    ...
    if vector.prior_attempts_to_resolve:
        ...
        weaknesses.append("Bring proof that you "
                          "tried to resolve the dispute before filing, if available.")
    ...
    if vector.counterclaim_present:
        weaknesses.append("There may be defenses or "
                          "counterclaims that reduce recovery or complicate the hearing.")

    if not strengths:
        strengths.append("The claim has enough stated detail"
                         " to support a structured presentation at hearing.")
    if not weaknesses:
        weaknesses.append("The main risk is whether"
    ...
    if vector.prior_attempts_to_resolve is False:
        advice_parts.append("If possible, bring a demand letter or other " \
